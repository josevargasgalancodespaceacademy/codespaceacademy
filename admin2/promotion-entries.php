...
<nav class="menu">
 ...
 <?php
                foreach ($company as $key => $company_contacts) {
                  echo "<tr><td>".$company[$key]["name"]."</td><td>".$company[$key]["surnames"]."</td><td>".$company[$key]["date_of_birth"]."</td><td>".$company[$key]["email"]."</td><td>".$company[$key]["number_identification"]."</td><td>".$company[$key]["telephone"]."</td><td>".$company[$key]["city"]."</td><td>".$company[$key]["comment"]."</td><td>".date("d-m-Y H:i:s", strtotime($company[$key]["created_at"]))."</td><td><form role='form' id='".$company[$key]["id"]."'><select class='formacion-a-medida' id='state".$company[$key]["id"]."' name='state'><option value=''>Selecciona el estado</option><option value='No contactado'>No contactado</option><option value='Imposible contactar'>Imposible contactar</option><option value='Contactado'>Contactado</option><option value='Entrevistado'>Entrevistado</option><option value='Presentado'>Presentado</option><option value='No presentado'>No presentado</option><option value='Beca OK'>Beca OK</option><option value='No Beca'>No Beca</option></select><button type='button' onClick='change_state(`".$company[$key]["id"]."`,`#state".$company[$key]["id"]."`,`promotion_entries`)' style='margin-left:20px'>Cambiar estado</button></form>".$company[$key]["state"]."</td></tr>";
                }
               ?>            
  </table>
</div>
