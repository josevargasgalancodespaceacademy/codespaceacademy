...
<nav class="menu">
 ...
 <?php
                foreach ($company as $key => $company_contacts) {
                  echo "<tr><td>".$company[$key]["name"]."</td><td>".$company[$key]["email"]."</td><td>".$company[$key]["telephone"]."</td><td>".date("d-m-Y H:i:s", strtotime($company[$key]["created_at"]))."</td><td><form role='form' id='company_contacts'><select class='formacion-a-medida' id='training_request' name='training_request'><option value=''>Estado</option><option value='No contactado'>No contactado</option><option value='Imposible contactar'>Imposible contactar</option><option value='Inscrito'>Inscrito</option><option value='No inscrito'>No inscrito</option></select><button type='button' style='margin-left:20px'>Cambiar estado</button></form></td></tr>";
                }
               ?>        
  </table>
  </div> 
